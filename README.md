...
본 레포지토리는 스마트 미터(AMI) 전력 데이터를 수집하고, <br>
딥러닝(LSTM)을 통해 미래 수요를 예측하는 파이프라인입니다.

## 필수 데이터 세팅 안내
본 프로젝트의 원본 데이터는 대용량으로, 드라이브에 압축하여 링크를 공유합니다.
...
**[데이터 설명]**
```
구하기 힘든 실제 1분 단위 전력 소비 데이터를 얻기 위해
가상데이터를 만들기보다는 실제 존재하는 데이터를 활용하였습니다.

프랑스 파리 전역 2075259가구의 1분 단위로 측정한 전력 소비 데이터에 
원주시 시간대별 전력 사용량 데이터의 전력 소비 패턴을 융합하여 1,000가구 분량의 실무형 데이터를 합성하였습니다.
```
